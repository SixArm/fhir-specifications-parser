...
pub struct RestSearchParam {
    ...
    /// Example : "http://hl7.org/fhir/SearchParameter/Account-owner"
    pub definition: String,

    /// Example: "reference"
    pub r#type: String,

    /// Example: "Entity managing the Account"
...
mod tests {
    use super::*;
    type T = RestSearchParam;

    #[test]
    fn test_serde_json_from_reader() {
        let path = crate::r5::parse::profiles_resources::DIR
            .join("rest_search_param")
            .join("rest_search_param.json");
        let file = std::fs::File::open(path).expect("open");
        let reader = std::io::BufReader::new(file);
        let actual: T = ::serde_json::from_reader(reader).unwrap();
        assert_eq!(actual.definition, String::from("http://hl7.org/fhir/SearchParameter/Account-owner"));
    }
}
